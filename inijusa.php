    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['asistencia'])) {
        ...
        foreach ($_POST['asistencia'] as $id_alumno => $datos_fechas) {
            ...
            if (!empty($updates)) {
                $sql_update = "UPDATE inijusa SET " . implode(", ", $updates) . " WHERE Alumno_id = ?"; // Ajusta Alumno_id a tu columna real de ID
    ...
    $sql_select = "SELECT " . $select_columns_sql ." FROM inijusa ORDER BY Alumno_id"; // Ajusta el nombre de tu tabla
    ...
    if ($resultado->num_rows > 0) {
        ...
        while($fila = $resultado->fetch_assoc()) {
            ...
            <td><a href="inijusa.php?id=<?=$fila['Alumno_id']?>&ta=inijusa">eliminar</a>
            </td>
            <?php 
            echo "</tr>";
        }
